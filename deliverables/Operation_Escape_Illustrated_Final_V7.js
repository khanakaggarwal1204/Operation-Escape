...
// Changes both the live alpha and the alpha remembered by the scene
// manager. Without this, show_scene() would restore a legacy object
// that had only been hidden with set_alpha().
...
// A compact four-point glint like the interaction markers used by
// classic 2D RPG mystery games. The hit area remains separate and can
// be much larger, but the visible effect never obscures the artwork.
function make_rpg_sparkle(list, x, y) {
    const vertical = make_rect(list, x, y, 3, 22, 255, 250, 226, 0);
    const horizontal = make_rect(list, x, y, 22, 3, 255, 250, 226, 0);
    const core = make_circle(list, x, y, 4, 255, 255, 248, 0);
    const mote_vertical = make_rect(list, x + 13, y - 11, 2, 8,
        255, 224, 142, 0);
    const mote_horizontal = make_rect(list, x + 13, y - 11, 8, 2,
        255, 224, 142, 0);
    return [vertical, horizontal, core, mote_vertical, mote_horizontal];
}

function position_rpg_sparkle(sparkle, x, y) {
    update_position(sparkle[0], [x, y]);
    update_position(sparkle[1], [x, y]);
    update_position(sparkle[2], [x, y]);
    update_position(sparkle[3], [x + 13, y - 11]);
    update_position(sparkle[4], [x + 13, y - 11]);
    return undefined;
}
...
function update_rpg_sparkle(sparkle, x, y, t, active, hovered) {
    ...
    if (!active) {
        for (let i = 0; i < array_length(sparkle); i = i + 1) {
            set_alpha(sparkle[i], 0);
        }
        return undefined;
    }
    ...
    const main_scale = (hovered ? 1.18 : 0.82) + 0.18 * wave;
    const mote_scale = 0.65 + 0.35 * (1 - wave);
...
function spawn_selected_character() {
    ...
    for (let p = 0; p < array_length(room1_player_parts); p = p + 1) {
        set_registered_alpha(room1_entries, room1_player_parts[p][0], 0);
    }
...
function draw_torch(list, x, y, phase) {
    ...
    const glow = make_rpg_sparkle(list, x, y - 34);
    ...
    return [flame_outer, flame_inner, glow, phase, x, y - 34];
}
...
function animate_hallway(torches, fog, rays, t) {
    for (let i = 0; i < array_length(torches); i = i + 1) {
        ...
        update_scale(flame_outer, [flicker, flicker]);
        update_scale(flame_inner, [flicker * 1.05, flicker * 0.95]);
        update_rpg_sparkle(glow, c[4], c[5], t, true, false);
    }
...
function animate_door(door, t, loop_count, hovered) {
    ...
    if (index === 1 && loop_count < door_unlock_flash_until) {
        update_rpg_sparkle(unlocked_glow, door[1] + 35,
            door[2], t, true, true);
    } else if (room_unlocked[index] && index !== 0) {
        update_rpg_sparkle(unlocked_glow, door[1] + 35,
            door[2], t + index, true, false);
    } else {
        update_rpg_sparkle(unlocked_glow, door[1] + 35,
            door[2], t, false, false);
    }
...
function play_key_animation(t, loop_count) {
    ...
    update_rpg_sparkle(room1_key_glow, ROOM1_CHEST_X,
        ROOM1_CHEST_Y + rise, t, true, key_reward_active(loop_count));

    if (elapsed >= 0 && elapsed < math_round(0.25 * FPS)) {
        const flash_p = 1 - elapsed / math_round(0.25 * FPS);
        update_color(room1_fade_rect, [255, 255, 255, 145 * flash_p]);
...
function make_interactable(list, name, x, y, range, ring_radius, visible_fn, prompt_label) {
    const sparkle = make_rpg_sparkle(list, x, y);
    room1_interactables[array_length(room1_interactables)] = [name, x, y, range, sparkle, visible_fn, prompt_label, ring_radius];
    return sparkle;
}
...
function draw_objects(list) {
    // The lamp and its lighting are already painted into the new
    // background. Do not draw the legacy vector candle over it.
    const candle = [];
    ...
    room1_key_glow = make_rpg_sparkle(list, ROOM1_CHEST_X,
        ROOM1_CHEST_Y);
    ...
    for (let i = 0; i < array_length(room1_interactables); i = i + 1) {
        ...
        if (visible_fn()) {
            ...
            if (dist < range || hovered) {
                ...
                update_rpg_sparkle(sparkle, x, y, t, false, false);
            }
        } else {
            update_rpg_sparkle(sparkle, x, y, t, false, false);
        }
...
function animate_room(candle, dust, window_glass, curtain, t, loop_count) {
    ...
    update_rpg_sparkle(room1_switch_glow, 415, 165, t,
        painting_p > 0, false);
...
function make_room2_interactable(list, name, x, y, range, ring_radius) {
    const sparkle = make_rpg_sparkle(list, x, y);
    room2_interactables[array_length(room2_interactables)] = [name, x, y, range, sparkle, ring_radius];
    return sparkle;
}
...
function assemble_room2_scene(list) {
    ...
    for (let p = 0; p < array_length(room2_player_parts); p = p + 1) {
        set_registered_alpha(list, room2_player_parts[p][0], 0);
    }
...
update_loop(game_state => {
    ...
    if (current_scene === "landing") {
        ...
        // A click above may have changed to the hallway. Never let the
        // remainder of this Room 1 frame redraw sprites over that scene.
        if (current_scene === "room1") {
            ...
            animate_room(room1_candle, room1_dust,
                room1_window_glass, room1_curtain, t, loop_count);
        }
        ...
        // The hallway button can change scenes during this branch.
        if (current_scene === "room2") {
